            ...
            <div class="col-md-10 offset-1">
                ...
                <table class="table table-striped">
                    ...
                    <tr><th v-for="(value, key) in data[0]"
                        ...
                        <a href="#">{{ key}} <span class="fa fa-sort"></span></a></th>
                    ...
                    <tr v-for="row in sortedData">
                        ...
                        <td>
                            <button class="btn btn-primary btn-sm" title="View Map"  @click="btnAction('view')">
                                <span class="fa fa-eye"></span>
                            </button>
                            <router-link class="btn btn-success btn-sm" title="Edit Map"
                                         :to="{path: '/map-edit', name: 'MapEditPage', params:{slug: row.slug, mapList: mapList}}">
                                <span class="fa fa-edit"></span>
                            </router-link>
                            <button class='btn btn-danger btn-sm'  title="Delete Map" @click="btnAction('delete')">
        ...
        methods:{
    		getData: function(){
                ...
                this.axios.get(uri).then((response) => {
                	...
                	if(response.data.length > 0){
                        ...
                        this.mapList = this.buildMapList(this.data)
                    } else
            ...
            postProcessData: function(data){
    		    data.forEach(function(row){
                    ...
                    row.Owner    = (row.owner == undefined || row.owner === "") ? 'Not Found' : row.owner
			        delete row.owner
                })
                return data
            },
            btnAction: function(action){
    			console.log(action)
                // syntax: this.$router.resolve({name: 'routeName', query: {data: "someData"}});
	            let routeData

                switch (action){
                    case 'view':
                        routeData = this.$router.resolve({path: '/map-view'});
